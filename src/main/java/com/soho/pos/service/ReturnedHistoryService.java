...
import com.soho.pos.dao.ReturnedHistoryDAO;
...
@Service
public class ReturnedHistoryService extends BaseService<ReturnedHistory, Long, ReturnedHistoryDAO> {
    ...
    public PageResult findPageQuery(ReturnedHistoryDTO rq) {
        StringBuilder sql = new StringBuilder();
        Map<String, Object> params = new HashMap<>();
        
        sql.append("select id, company_id, buy_no, count(id), status, company_order_no, returned_date from returned_history where 1 = 1 ");
    
        if (StringUtils.isNotBlank(rq.getDate())) {
            String sDate = rq.getDate().split("至")[0].trim();
            String eDate = rq.getDate().split("至")[1].trim();
            sql.append(" and returned_date >= :sDate ");
            params.put("sDate", LocalDate.parse(sDate));
        
            sql.append(" and returned_date <= :eDate ");
        ...
        sql.append(" order by returned_date desc ");
        
        Query query = em.createNativeQuery(sql.toString());
        
        SqlUtils.getQueryWithParameters(query, params);
        List<Object[]> resultList = query.getResultList();
        List<ReturnedHistoryVO> voList = new ArrayList<>();
        for (Object[] result : resultList) {
            ...
            String returnedDate = result[6].toString();
            ReturnedHistoryVO vo = new ReturnedHistoryVO();
            vo.setId(id);
            vo.setNum(num);
            vo.setStatus(status);
            ...
            vo.setReturnedDate(returnedDate);
            voList.add(vo);
        }
    ...
    public void saveToInventory(String buyNo) throws PortalException {
        ...
        List<ReturnedHistory> historyList = dao.findByBuyNo(buyNo);
    ...
    public PageResult findByDataInfo(ReturnedHistoryDTO rq) {
        List<ReturnedHistory> historyList = dao.findByBuyNo(rq.getBuyNo());
        List<ReturnedVO> voList = ReturnedHistoryMapper.INSTANCE.to(historyList);
        long total = voList.stream().mapToInt(x -> (x.getCnt() * x.getPrice())).sum();
        long tariffPay = 0;
        for (ReturnedVO vo : voList) {
            int cnt = vo.getCnt();
    ...
    public void del(Long id) throws PortalException {
        ReturnedHistory history = dao.findById(id).orElseThrow(() -> new PortalException(ErrorText.ID_FAIL.getMsg()));
        Inventory inventory = inventoryDAO.findByProductIdAndSizeId(history.getProductId(), history.getSizeId());
        if (inventory != null && "1".equals(history.getStatus())) {
            int cnt = inventory.getCnt() + history.getCnt();
            inventory.setCnt(cnt);
