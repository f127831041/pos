...
function init() {
    ...
    let url = utils.getHandlerPath("/revenue/query");
    let customObj = {
        order: [0, 'desc'],
        columns: getColumns()
    };
...
function dialogOpen(id) {
    $('#saleListModal').modal({backdrop: 'static', keyboard: false});
    $('#saleListModal').modal('show');
    let url = utils.getHandlerPath("/sale/query");
    $("#revenueId").val(id);
    let customObj = {
        order: [0, 'desc'],
        columns: getSaleColumns()
    };
    getDataTableQuery(url, $("#dataTable2"), $("#saleListForm"), customObj);
}
...
function getColumns() {
    return [
        ...
        {
            data: "discount", title: "折扣金額", className: "text-center",
            "render": function (data, type, row, meta) {
                return data.toLocaleString();
            }
        },
        {
            data: "bonus", title: "紅利抵用金", className: "text-center",
            "render": function (data, type, row, meta) {
                return data.toLocaleString();
            }
        },
        {
            data: "promotion", title: "門市優惠", className: "text-center",
            "render": function (data, type, row, meta) {
                return data.toLocaleString();
            }
        },
        ...
        {
            data: "points", title: "紅利回饋", className: "text-center",
            "render": function (data, type, row, meta) {
                return data.toLocaleString();
            }
        },
