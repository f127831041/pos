    ...
    let option = $.extend(utils.dataTableOption(), {
        ajax: function (data, callback, settings) {
            ...
            $.ajax({
                ...
                if(url.indexOf("/returnedHistory/data/query") > 0){
...
function init() {
    ...
    let url = utils.getHandlerPath("/returnedHistory/query");
    let customObj = {
        order: [0, 'desc'],
        columns: getColumns()
    };
...
function dialogOpen(buyNo) {
    ...
    let url = utils.getHandlerPath("/returnedHistory/data/query");
    let customObj = {
        order: [0, 'desc'],
...
function getColumns() {
    return [
        ...
        {data: "returnedDate", title: "退貨日期", className: "text-center", sortable: false, orderable: false},
        ...
        {data: "num", title: "品項數量", className: "text-center", sortable: false, orderable: false},
        {
            ...
            "render": function (data, type, row, meta) {
                let inventory = `<button type='button' class='btn btn-sm btn-success' onclick='saveInventory("${data.buyNo}")'><i class="fa fa-cart-plus"></i>轉出庫存</button>`;
                return data.status === '2' ? inventory : '已轉出';
            }
        },
        {
            data: null,
...
function deleteRow(id) {
    let url = "/returnedHistory/delete/" + id
...
function saveInventory(buyNo) {
    ...
    $.ajax({
        type: "post",
        url: utils.getHandlerPath("/returnedHistory/save/inventory"),
