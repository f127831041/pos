 ...
 */
function init() {
    let url = utils.getHandlerPath("/member/query");
    let customObj = {
        order: [0, 'desc'],
        columns: getColumns('')
    };
...
function dialogOpen(id) {
    ...
    $.ajax({
        ...
        $('#memberModal').find('.modal-content').html(response);
    }).always(function () {
        $('#memberModal').modal({backdrop: 'static', keyboard: false})
        $('#memberModal').modal('show');
        $('#memberModal').find('.select2').select2({
            theme: "bootstrap-5",
...
function getColumns() {
    return [
        ...
        {
            data: "birthday", title: "生日", className: "text-center", "render": function (data, type, row, meta) {
                let month = utils.isEmpty(row.month) ? '' : row.month;
                let day = utils.isEmpty(row.day) ? '' : '/' + row.day;
                return month + day;
            }
        },
        {
            data: "points", title: "紅利點數", className: "text-center", "render": function (data, type, row, meta) {
...
function save() {
    ...
    $('#memberForm').validate({
        ...
        rules: {
            ...
            let url = actionType === 'add' ? "/member/add" : "/member/update";
            let reset = actionType === 'add';
            let param = $("#memberForm").serializeObject();
            $.ajax({
                type: "post",
                url: utils.getHandlerPath(url),
